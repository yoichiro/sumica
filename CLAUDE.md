...
For **SD1.5** the main form keeps the original two `<select>`s (幅 / 高さ) backed by `SD15_SIZE_OPTIONS = [512, 768, 1024]`, plus the ArrowLeftRight swap button. SD1.5 wasn't trained with aspect-ratio buckets, so the preset abstraction offers no fidelity benefit there.
...
- **SD1.5**: cross product of width × height picked from `SD15_SIZE_OPTIONS` = `[512, 768, 1024]`, exactly like the main form's dropdowns.
