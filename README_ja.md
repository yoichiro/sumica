...
![Sumica AI Studio のスクリーンショット](sumica-screenshot-ja.png)
...
- タイルから **「フォームにロード」** で過去の生成設定を復元 (モデル・サイズ・Seed・Sampler・LoRA など)。左パネルがランキングタブの状態でロードすると、自動的にフォームタブに切り替わって設定反映を確認できます。

### 🏆 ランキング

- 左パネルの生成フォームと並んで **🏆 ランキング** タブを配置。⭐ お気に入りに登録した画像から抽出した「レシピ」（モデル / サイズ / Sampler / Scheduler / Steps / CFG / Hires / LoRA / Refiner / VAE の組み合わせ）を Top-N 形式で表示します。並び順は絶対お気に入り数の降順（[ADR 35](docs/arch/adr-0035-ranking-order-by-absolute-favs.md) 参照）。
- 各行に **SDXL / SD1.5 アーキテクチャチップ**、アスペクト比を視覚化した矩形プレビュー、そして **「フォームに適用」** ボタンを配置。ワンクリックで全パラメータを生成フォームに書き戻せます。
