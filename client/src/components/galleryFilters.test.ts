...
describe('applyGalleryFilters', () => {
  ...
  it('prefers persisted modelArchitecture over title inference when present', () => {
    ...
    // (ADR 16); it should win even if the checkpoint title/known-model lookup
    // would otherwise resolve differently (e.g. a renamed or unlisted model).
    const history = [
      mkRecord({ model: 'renamedCheckpoint.safetensors', modelArchitecture: 'sdxl' }),
      mkRecord({ model: 'renamedCheckpoint.safetensors' }),
    ];
    const out = applyGalleryFilters(history, { ...ALL_NULL, arch: 'sdxl' }, []);
    expect(out).toHaveLength(1);
    expect(out[0].modelArchitecture).toBe('sdxl');
  });
