...
承認済み — 以下 2 件により部分的に amended されています。

- [[adr-0024-ranking-recipe-full-form-restore]] により recipe key の次元セットが拡張されました（8 次元 → 15 次元 + LoRA weight）。
- [[adr-0035-ranking-order-by-absolute-favs]] により並び順が Wilson 下限降順から「お気に入り絶対数の降順」に変更されました。`wilsonLower()` 関数、`RankedRecipe.wilson` / `rate`、および Wilson% / Rate% の UI 表示は完全削除されています。

rollup の仕組み・書き込みフロー・rollup コレクションのスキーマ・UI 構造は本 ADR のまま生きており、変更は「recipe key の次元セットと並び順アルゴリズム」に限定されます。
