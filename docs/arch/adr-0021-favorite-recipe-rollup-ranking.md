...
承認済み — [[adr-0024-ranking-recipe-full-form-restore]] により recipe key の次元セットが拡張されました（8 次元 → 15 次元 + LoRA weight）。rollup の仕組み・書き込みフロー・分析ロジック・UI 構造は本 ADR のまま生きており、変更は「recipe key に含める次元と、それにともなう `applyRecipe()` の復元範囲」に限定されます。
