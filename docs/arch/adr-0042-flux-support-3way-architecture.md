...
非推奨 (Deprecated)。[[adr-0054-rollback-forge-neo-and-flux-to-automatic1111]] にて Flux 対応と 3-way トグルを撤回し、SD1.5 / SDXL の 2-way に戻しました。撤回の背景は ADR-54 の Context を参照してください。本 ADR の内容は履歴として保持しますが、現行コードには反映されていません。
...
- **AUTOMATIC1111 v1.10 の Flux VAE サポートは実質不完全**: 本 ADR は AUTOMATIC1111 v1.10+ を前提としましたが、実運用で AUTOMATIC1111 v1.10 は Flux VAE を SD の `AutoencoderKL` に読み込もうとして `state_dict` shape mismatch でクラッシュすることが判明しました。この問題は Sumica のコード側では解決不可能で、Stable Diffusion WebUI Forge Neo Classic への移行が実用解になります。Forge Neo Classic への対応は [[adr-0052-forge-neo-preset-sync]] で扱います。
